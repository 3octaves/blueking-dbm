...
<template>
  <tr>
    ...
    <td style="padding: 0">
      <RenderText
        :data="data.clusterData?.masterSlaveList.join(',')"
        :is-loading="data.isLoading"
        :placeholder="t('选择集群后自动生成')">
        <template #content>
          <p
            v-for="item in data.clusterData?.masterSlaveList"
            :key="item">
            {{ item }}
          </p>
        </template>
    ...
    <td style="padding: 0">
      <RenderText
        :data="data.clusterData?.readonlySlaveList.join(',')"
        :is-loading="data.isLoading"
        :placeholder="t('选择集群后自动生成')">
        <template #content>
          <p
            v-for="item in data.clusterData?.readonlySlaveList"
            :key="item">
            {{ item }}
          </p>
        </template>
...
<script lang="ts">
  ...
  export interface IDataRow {
    ...
    clusterData?: {
      ...
      moduleName: string;
      cloudId: number;
      masterSlaveList: string[];
      readonlySlaveList: string[];
    };
