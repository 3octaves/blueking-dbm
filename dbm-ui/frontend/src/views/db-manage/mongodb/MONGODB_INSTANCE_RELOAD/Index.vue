  ...
  <SmartAction class="db-toolbox">
    ...
    <BkForm
      ...
      <BkFormItem
        :label="t('目标选择模式')"
        required>
        <BkRadioGroup v-model="formData.targetSelectMode">
          <BkRadio label="cluster">
            {{ t('按集群') }}
          </BkRadio>
          <BkRadio label="machine">
            {{ t('按主机') }}
          </BkRadio>
          <BkRadio label="instance">
            {{ t('按实例') }}
          </BkRadio>
        </BkRadioGroup>
      </BkFormItem>
...
<script setup lang="ts">
  ...
  const currentTableRef = useTemplateRef('currentTableRef');

  // 默认表单数据
  const defaultData = () => ({
    force: false,
    payload: createTicketPayload(),
    targetSelectMode: 'cluster' as Mongodb.InstanceReload['target_select_mode'],
  });
  ...
  useTicketDetail<Mongodb.InstanceReload>(TicketTypes.MONGODB_INSTANCE_RELOAD, {
    onSuccess(ticketDetail) {
      ...
      Object.assign(formData, {
        isForceReload: details.force ?? false,
  ...
  const handleSubmit = async () => {
    ...
    const result = await tableRef.validate();
    if (!result) {
      return;
    }

    const infos = currentTableRef.value.getValue();
