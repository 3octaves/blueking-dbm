...
<script setup lang="ts">
  ...
  useTicketDetail<Mongodb.InstanceReload>(TicketTypes.MONGODB_INSTANCE_RELOAD, {
    onSuccess(ticketDetail) {
      ...
      const { mackine_infos: machineInfos, target_select_mode: targetSelectMode } = details;
      const infos = details.infos as { bk_host_id: number }[];

      if (targetSelectMode === 'machine') {
        tableData.value = infos.map((item) => createRow({ host: { ip: machineInfos[item.bk_host_id].ip } }));
      }
    },
  });
