...
// MongoDB 扩容接入层
export async function generateProxyScaleUpCloneData(ticketData: TicketModel<ProxyScaleUp>) {
  ...
  const tableDataList = infos.map((item) => {
    ...
    return {
      ...
      currentSpec: {
        ...
        count: clusterItem.shard_num,
      },
      targetNum: String(item.resource_spec.mongos.count || ''),
    };
  });
