...
import type { ShardScaleDown } from '@services/model/ticket/details/mongo';
...
// MongoDB 缩容Shard节点数
export async function generateReduceShardNodesCloneData(ticketData: TicketModel<ShardScaleDown>) {
  ...
  const tableDataList = infos.map((item) => {
    ...
    return {
      ...
      targetNum: String(item.reduce_shard_nodes || ''),
    };
  });
