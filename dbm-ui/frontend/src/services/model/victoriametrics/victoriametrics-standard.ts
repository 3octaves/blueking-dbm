...
export default class VictoriametricsStandard extends ClusterBase {
  static operationIconMap: Record<string, string> = {
    [TicketTypes.K8S_VICTORIAMETRICS_DELETE]: t('删除中'),
  ...
  static operationTextMap: Record<string, string> = {
    [TicketTypes.K8S_VICTORIAMETRICS_DELETE]: t('删除任务进行中'),
  ...
  permission: {
    ...
    k8s_victoriametrics_enable_disable: boolean;
  ...
  get isOfflineOperationRunning() {
    return ([TicketTypes.K8S_VICTORIAMETRICS_ENABLE, TicketTypes.K8S_VICTORIAMETRICS_DELETE] as string[]).includes(
  ...
  get queryEntryDisplay() {
    return this.queryEntryDomain ? `${this.queryEntryDomain}:8481` : '';
  }

  get queryEntryDomain() {
    return this.getClbEntryByRole('query')?.entry || '';
  }
  ...
  get storageEntryDisplay() {
    return this.storageEntryDomain ? `${this.storageEntryDomain}:8482` : '';
  }

  get storageEntryDomain() {
    return this.getClbEntryByRole('storage')?.entry || '';
  }

  get writeEntryDisplay() {
    return this.writeEntryDomain ? `${this.writeEntryDomain}:8480` : '';
  }

  get writeEntryDomain() {
    return this.getClbEntryByRole('write')?.entry || '';
  }

  /**
   * VictoriaMetrics 标准集群多入口（CLB）：写入入口 role='write'(8480)、查询入口 role='query'(8481)、
   * 存储入口 role='storage'(8482)。前端按 role 固定端口，cluster_entry 内无端口字段。
   */
