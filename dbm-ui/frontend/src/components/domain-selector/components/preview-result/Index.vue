      ...
      <BkDropdown class="result-dropdown">
        ...
        <template #content>
          <BkDropdownMenu>
            ...
            <BkDropdownItem @click="handleCopyDomains">
              {{ t('复制所有域名') }}
            </BkDropdownItem>
          </BkDropdownMenu>
        </template>
...
<script setup lang="ts" generic="T extends ISupportKey">
  ...
  import type { ISupportKey } from '../../types';
  ...
  const isEmpty = computed(() => Object.values<IRowData[]>(props.lastValues).every((values) => values.length === 0));
  const showTitle = computed(() => props.keys.length > 1);

  const handleClear = () => {
    if (isEmpty.value) {
      return;
    }
    emits('change', Object.fromEntries(props.keys.map((key) => [key, [] as IRowData[]])) as Props<T>['lastValues']);
  };
  ...
  const handleCopyDomains = () => {
    ...
    const copyData = Object.values<IRowData[]>(props.lastValues).flatMap((lastValuesItem) =>
      lastValuesItem.map((item) => item.domain),
    );
